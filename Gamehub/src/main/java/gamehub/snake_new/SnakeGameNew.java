...
public class SnakeGameNew extends JPanel implements ActionListener, KeyListener {
    ...
    private void updateGame() {
        ...
        if (!freezeActive) {
            if (newX<0||newX>=GRID_WIDTH||newY<0||newY>=GRID_HEIGHT) { running=false; gameOver=true; return; }
        } else {
        ...
        if (!shieldActive) {
            for (int i=0; i<snake.size(); i++) {
                ...
                if (s.gridX==newX && s.gridY==newY) { running=false; gameOver=true; return; }
            }
        }
    ...
    private void applyFruitEffect(FruitType type) {
        ...
        switch (type) {
            ...
            case ROTTEN_MEAT  -> {
                // Schlange schrumpft um 3 (mindestens 3 Segmente bleiben)
                for (int i=0; i<3 && snake.size()>3; i++) snake.remove(snake.size()-1);
                // Score um 3 reduzieren, Minimum 0
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (paused && running) drawCenteredOverlay(g2, "PAUSED",    "SPACE = Weiter");
        if (gameOver)          drawCenteredOverlay(g2, "GAME OVER", "R = Neu starten");
    ...
    /**
     * Zeichnet die Schlange. null-Sentinels in der Punkteliste markieren Wrap-Brüche —
     * an diesen Stellen wird der Pfad getrennt, sodass kein Strich über den Rand geht.
     */
    private void drawSnakeOrganic(Graphics2D g2, float progress, double time) {
        // getRenderPoints gibt null-Sentinels bei Wrap-Sprüngen zurück
        List<RenderPoint> pts = getInterpolatedSnakeCenters(progress);

        // Slither-Wellen aufbauen; null-Sentinels durchreichen
        List<RenderPoint> slithered = new ArrayList<>();
        for (int i=0; i<pts.size(); i++) {
            RenderPoint p = pts.get(i);
            if (p == null) { slithered.add(null); continue; }

            // Nachbarn für Normalenvektor suchen (Sentinels überspringen)
            RenderPoint prev=null, next=null;
            for (int j=i-1; j>=0; j--)          { if (pts.get(j)!=null){prev=pts.get(j);break;} }
            for (int j=i+1; j<pts.size(); j++)   { if (pts.get(j)!=null){next=pts.get(j);break;} }

            double nx=0, ny=0;
            if (next!=null) {
                double dx=next.x-p.x, dy=next.y-p.y, len=Math.hypot(dx,dy);
                if (len>0.0001) { nx=-dy/len; ny=dx/len; }
            } else if (prev!=null) {
                double dx=p.x-prev.x, dy=p.y-prev.y, len=Math.hypot(dx,dy);
                if (len>0.0001) { nx=-dy/len; ny=dx/len; }
            }
            double amp  = Math.max(0.0, 3.8-i*0.33);
            double wave = Math.sin(time*8.5-i*0.8)*amp;
            float sx = (i==0) ? p.x : (float)(p.x+nx*wave);
            float sy = (i==0) ? p.y : (float)(p.y+ny*wave);
    ...
    /**
     * Gibt interpolierte Pixel-Positionen zurück.
     *
     * Bei einem Wrap-Sprung (|delta| > 1, nur im Freeze-Modus möglich) wird der
     * Animations-Schritt in zwei Hälften aufgeteilt:
     *   progress < 0.5  → Segment gleitet aus dem aktuellen Rand heraus
     *   null-Sentinel   → Pfad-Bruch: kein Strich quer durch den Bildschirm
     *   progress >= 0.5 → Segment kommt auf der gegenüberliegenden Seite herein
     */
    private List<RenderPoint> getInterpolatedSnakeCenters(float progress) {
        ...
        for (SnakeSegment s : snake) {
            int ddx = s.gridX - s.prevGridX;
            int ddy = s.gridY - s.prevGridY;
            boolean wrapX = Math.abs(ddx) > 1;
            boolean wrapY = Math.abs(ddy) > 1;

            if (wrapX || wrapY) {
                // exitDir: Richtung, in die das Segment den Rand verlässt
                float exX = wrapX ? (ddx < 0 ? -1f : 1f) : 0f;
                float exY = wrapY ? (ddy < 0 ? -1f : 1f) : 0f;

                if (progress < 0.5f) {
                    // Phase 1: Segment gleitet aus dem Rand heraus (0 → Kante)
                    float t2 = progress * 2f;
                    float gx = s.prevGridX + exX * t2;
                    float gy = s.prevGridY + exY * t2;
                    pts.add(new RenderPoint(bx+gx*ts+ts/2f, by+gy*ts+ts/2f));
                } else {
                    // Pfad-Bruch: kein Strich über den Bildschirm
                    pts.add(null);
                    // Phase 2: Segment kommt von der Gegenseite herein (Kante → Ziel)
                    float t2 = (progress-0.5f)*2f;
                    float gx = s.gridX - exX*(1f-t2);
                    float gy = s.gridY - exY*(1f-t2);
                    pts.add(new RenderPoint(bx+gx*ts+ts/2f, by+gy*ts+ts/2f));
                }
            } else {
    ...
    private void drawSnakeShadow(Graphics2D g2, List<RenderPoint> pts) {
        int t=getTileSize();
        ...
        for (Path2D.Float p : buildSubPaths(pts)) g2.draw(p);
        g2.setTransform(old);
    }

    private void drawSnakeBody(Graphics2D g2, List<RenderPoint> pts) {
        int t=getTileSize();
        List<Path2D.Float> paths=buildSubPaths(pts);
        SnakeColor c=col();
        if (shieldActive) {
            ...
            for (Path2D.Float p:paths) g2.draw(p);
        }
        if (speedBoost) {
    ...
    private void drawSnakePattern(Graphics2D g2, List<RenderPoint> pts) {
        int t=getTileSize();
    ...
    /**
     * Baut aus einer Punkt-Liste (mit null-Sentinels) mehrere unabhängige Pfade.
     * Jeder null-Eintrag beendet den aktuellen Pfad und beginnt einen neuen.
     * So wird verhindert, dass eine Linie quer über den Bildschirm gezeichnet wird.
     */
    private List<Path2D.Float> buildSubPaths(List<RenderPoint> pts) {
        List<Path2D.Float> result = new ArrayList<>();
        List<RenderPoint>  seg    = new ArrayList<>();
        for (RenderPoint p : pts) {
            if (p == null) {
                if (!seg.isEmpty()) { result.add(buildSmoothPath(seg)); seg=new ArrayList<>(); }
            } else {
                seg.add(p);
            }
        }
        if (!seg.isEmpty()) result.add(buildSmoothPath(seg));
        return result;
    }
    ...
    private void drawSnakeHead(Graphics2D g2, List<RenderPoint> pts, double time) {
        RenderPoint head=null;
        for (RenderPoint p:pts) if(p!=null){head=p;break;}
        if (head==null) return;
    ...
    private void drawEyes(Graphics2D g2, double time, int t) {
        ...
        if (blink) {
            ...
            g2.draw(new Line2D.Float(rx-2,ry,rx+2,ry));
            return;
        }
    ...
    private void drawTongue(Graphics2D g2, double time, int t) {
        if (!isTongueOut(time)||!running||gameOver||paused) return;
    ...
    private void drawApple(Graphics2D g,int x,int y,int t){
        g.setColor(new Color(216,50,50));
        g.fillOval(x+t/5,y+t/4,t/2,t/2); g.fillOval(x+t/2,y+t/4,t/2,t/2);
    ...
    private void drawPear(Graphics2D g,int x,int y,int t){
        g.setColor(new Color(184,214,77));
        g.fillOval(x+t/3,y+t/6,t/3,t/3); g.fillOval(x+t/5,y+t/2-t/10,t*3/5,t/2);
    ...
    private void drawCherry(Graphics2D g,int x,int y,int t){
        ...
        g.setColor(new Color(70,180,75));
        g.drawLine(x+t/3,y+t/2,x+t/2,y+t/5); g.drawLine(x+t*2/3,y+t/2,x+t/2,y+t/5);
    }
    private void drawGoldApple(Graphics2D g,int x,int y,int t,double time){
        float pulse=1f+(float)Math.sin(time*5)*0.08f;
        int cx=x+t/2,cy=y+t/2,r=(int)(t*0.38f*pulse);
        ...
        g.setStroke(new BasicStroke(Math.max(1.5f,t*0.07f)));
        g.drawLine(cx,cy-r,cx+t/6,cy-r-t/5);
        g.setColor(new Color(80,180,50)); g.fillOval(cx+t/10,cy-r-t/4,t/5,t/8);
        int gs=Math.max(2,t/8);
        g.setColor(new Color(255,255,180,200)); g.fillOval(cx+r/2-gs/2,cy-r/2-gs/2,gs,gs);
    }
    private void drawRottenApple(Graphics2D g,int x,int y,int t,double time){
        int cx=x+t/2,cy=y+t/2,r=(int)(t*0.35f);
        float w=(float)Math.sin(time*4)*2;
        g.setColor(new Color(110,20,140));   g.fillOval(cx-r+(int)w,cy-r,r*2,r*2);
        g.setColor(new Color(60,10,80));
        g.fillOval(cx-r/3,cy-r/4,r/2,r/3); g.fillOval(cx+r/4,cy+r/4,r/3,r/4);
        ...
        g.setFont(new Font("SansSerif",Font.BOLD,Math.max(8,t/3)));
    ...
    private void drawRottenMeat(Graphics2D g,int x,int y,int t,double time){
        ...
        g.fillOval(x+t/4,y+t/3,t/5,t/5);
        g.fillOval(x+t/2,y+t/2-t/8,t/6,t/6);
        g.fillOval(x+t/3,y+t*2/3-t/8,t/7,t/7);
        double fa=time*4;
        int fx=cx+(int)(Math.cos(fa)*t*0.38), fy=cy+(int)(Math.sin(fa)*t*0.25)-t/4;
    ...
    private void drawLightning(Graphics2D g,int x,int y,int t,double time){
        int cx=x+t/2,cy=y+t/2;
        float fl=(float)(0.7+Math.sin(time*20)*0.3);
    ...
    private void drawIceFruit(Graphics2D g,int x,int y,int t,double time){
        int cx=x+t/2,cy=y+t/2,r=(int)(t*0.36f);
        float sp=(float)(0.5+Math.sin(time*6)*0.5);
        g.setColor(new Color(150,220,255,200)); g.fillOval(cx-r,cy-r,r*2,r*2);
        g.setColor(new Color(200,240,255,200));
        g.setStroke(new BasicStroke(Math.max(1f,t*0.07f)));
        for (int i=0;i<6;i++){
            double a=i*Math.PI/3;
            g.draw(new Line2D.Float(cx,cy,cx+(float)(Math.cos(a)*r*0.85f),cy+(float)(Math.sin(a)*r*0.85f)));
        }
        g.setColor(new Color(255,255,255,(int)(180*sp))); g.fillOval(cx-r/2,cy-r*2/3,r*2/3,r/3);
        int gs=Math.max(2,t/10);
        g.setColor(new Color(255,255,255,(int)(220*sp)));
    ...
    private void drawCenteredOverlay(Graphics2D g2, String title, String subtitle) {
        ...
        int boxW=Math.min(280,bw-40),boxH=120,boxX=bx+(bw-boxW)/2,boxY=by+(bh-boxH)/2;
        g2.setColor(new Color(28,36,28,220)); g2.fillRoundRect(boxX,boxY,boxW,boxH,24,24);
        g2.setColor(new Color(255,255,255,35)); g2.drawRoundRect(boxX,boxY,boxW,boxH,24,24);
        g2.setColor(Color.WHITE); g2.setFont(new Font("SansSerif",Font.BOLD,28));
        FontMetrics fm=g2.getFontMetrics();
        g2.drawString(title, bx+(bw-fm.stringWidth(title))/2, boxY+45);
        g2.setFont(new Font("SansSerif",Font.PLAIN,16)); fm=g2.getFontMetrics();
        g2.drawString(subtitle, bx+(bw-fm.stringWidth(subtitle))/2, boxY+80);
    }
    ...
    public void keyPressed(KeyEvent e) {
        switch (e.getKeyCode()) {
            case KeyEvent.VK_UP,    KeyEvent.VK_W -> { if (currentDirection!=Direction.DOWN)  nextDirection=Direction.UP; }
            case KeyEvent.VK_DOWN,  KeyEvent.VK_S -> { if (currentDirection!=Direction.UP)    nextDirection=Direction.DOWN; }
            case KeyEvent.VK_LEFT,  KeyEvent.VK_A -> { if (currentDirection!=Direction.RIGHT) nextDirection=Direction.LEFT; }
            case KeyEvent.VK_RIGHT, KeyEvent.VK_D -> { if (currentDirection!=Direction.LEFT)  nextDirection=Direction.RIGHT; }
            case KeyEvent.VK_SPACE -> { if (running&&!gameOver) paused=!paused; }
...
class SettingsPanel extends JPanel {
    ...
    private void drawColorDots(Graphics2D g2, Rectangle r) {
        ...
        for (int i=0; i<total; i++) {
            SnakeGameNew.SnakeColor c=SnakeGameNew.SNAKE_COLORS[i];
            int dx=sx+i*sp;
    ...
    private void drawModeCard(Graphics2D g2) {
        ...
        String desc=chaosMode?"Spezialfruchte aktiv!":"Klassisches Snake";
    ...
    private void drawModeIcon(Graphics2D g2, Rectangle r) {
        ...
        if (!chaosMode) {
            ...
            g2.fillOval(cx-10,cy-8,12,12); g2.fillOval(cx-2,cy-8,12,12);
